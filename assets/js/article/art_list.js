$(function() {
    var layer = layui.layer
    var form = layui.form
    var laypage = layui.laypage


    //定义一个查询的参数对象，将来请求数据的时候，需要将请求的参数对象提交到服务器
    var q = {
        pagenum: 1, //页码值，默认请求第一页的数据
        pagesize: 5, //每页显示几条数据，默认每页显示2条
        cate_id: '', //文章分类的Id
        state: '' //文章的发布状态
    }

    initTable()
    initCate()

    // 定义一个美化时间的过滤器
    template.defaults.imports.dataFormat = function(date) {
        const dt = new Date(date)

        var y = dt.getFullYear()
        var m = padZero(dt.getMonth() + 1)
        var d = padZero(dt.getDate())

        var hh = padZero(dt.getHours())
        var mm = padZero(dt.getMinutes())
        var ss = padZero(dt.getSeconds())

        return y + '-' + m + '-' + d + ' ' + hh + ':' + mm + ':' + ss
    }

    // 定义补零函数
    function padZero(n) {
        return n > 9 ? n : '0' + n
    }

    // 获取文章列表的方法
    function initTable() {
        console.log(q)
        $.ajax({
            method: 'GET',
            url: '/my/article/list',
            data: q,
            success: function(res) {
                // console.log(res);
                if (res.status !== 0) {
                    return layer.msg('获取文章列表失败')
                }
                //使用模板引擎渲染页面的数据
                var htmlStr = template('tpl-table', res)
                $('tbody').html(htmlStr)
                    // 调用渲染分页的方法
                renderPage(res.total)

            }
        })
    }

    // 初始化文章分类的方法,下拉框获取分类数据
    function initCate() {
        $.ajax({
            method: 'GET',
            url: '/my/article/cates',
            success: function(res) {
                if (res.status !== 0) {
                    return layer.msg('获取数据失败！！')
                }
                layer.msg('获取数据成功！！')
                var htmlStr = template('tpl-cate', res)
                    // console.log(htmlStr);
                $('[name=cate_id]').html(htmlStr)
                    // 通过layui重新渲染表单区域的UI结构
                form.render()
            },
        })
    }

    //为筛选表单绑定submit事件
    $('#form-search').on('submit', function(e) {
        e.preventDefault()
            // 获取表单中选中项的值
        var cate_id = $('[name=cate_id]').val()
        var state = $('[name=state_id]').val()
            // 为查询参数对象q中对应的属性赋值
        q.cate_id = cate_id
        q.state = state
            //根据最新的筛选条件，重新渲染不过的数据
        initTable()
    })

    // 定义渲染分页的方法
    // BUG：每日定义一个方法函数后，第一件事情是先调用，避免遗忘；
    // BUG: 加不必要的引号，根据文档，除了elem用单引号，其他都是数值型
    function renderPage(total) {
        console.log(total);
        // 错误代码：误加引号
        // 调用laypage.render()方法来渲染分页的结构
        // laypage.render({
        //     elem: 'pageBox', //存放分页的容器ID
        //     count: 'total', //数据总数
        //     limit: 'q.pagesize', //每页显示的条数
        //     curr: 'q.pagenum', //页码值，默认请求第一页的数据

        // })

        laypage.render({
            elem: 'pageBox', //存放分页的容器ID
            count: total, //数据总数
            limit: q.pagesize, //每页显示的条数
            curr: q.pagenum, //页码值，默认请求第一页的数据
            //分页栏美化优化
            layout: ['count', 'limit', 'prev', 'page', 'next', 'skip'],
            limits: [2, 3, 5, 8, 10],

            // 分页切换时，触发jump事件
            // 触发jump有两种方式：
            // 1.点击页码时候，会触发jump回调
            // 2.只要调用laypage.render()方法，就会触发jump回调
            jump: function(obj, first) {
                // 可以通过first的值，判断是通过哪种方式，触发的jump回调
                // 如果first的值为true，证明是方式2触发的
                // 否则就是方式1触发的，undefined
                console.log(first);
                console.log(obj.curr);
                // 把最新的页码值，赋值给q的这个查询参数对象中
                q.pagenum = obj.curr
                    //除了切换分页，点击每页显示条数的按钮也能触发jump事件
                    //  把最新的条目数，赋值到q这个查询参数对象的pagesize属性中   
                q.pagesize = obj.limit


                if (!first) {
                    initTable()
                }
            }

        })
    }

    // 通过代理的方式，为删除按钮绑定点击事件处理函数
    $('tbody').on('click', '.btn-delete', function() {

        // 获取删除按钮的个数
        var len = $('.btn-delete').length
        console.log(len);

        // 获取到文章的Id
        var Id = $(this).attr('data-id')

        //询问用户是否要删除数据
        layer.confirm('确认删除？', { icon: 3, title: '提示' }, function(index) {

            $.ajax({
                method: 'GET',
                url: '/my/article/delete/' + Id,
                success: function(res) {
                    if (res.status !== 0) {
                        return layer.msg('删除文章失败')
                    }
                    layer.msg('删除文章成功')
                    if (len === 1) {
                        // 如果len的值等于1，证明删除完毕之后，页面上就没有其他任何数据
                        // 页码值最小必须是1（三元表达式判断）
                        q.pagenum = q.pagenum === 1 ? 1 : q.pagenum - 1

                    }
                    initTable()
                },
            })

            layer.close(index);
        });
    })
})